...
function ENT:OnSpawn( PObj )
	--[[ basics ]]
	local Pod = self:AddDriverSeat( Vector(-20.86, 10, 34), Angle(0,0,0) ) -- self:AddDriverSeat( Position,  Angle ) -- add a driver seat (max 1)
	...
	self:AddEngine( Vector(0,70,10) ) -- add a engine. This is used for sounds and effects and is required to get accurate RPM for the gauges.
	...
	--Example:
	self:AddFuelTank( Vector(0,-30,5), Angle(0,0,0), 1200, LVS.FUELTYPE_PETROL )
...
-- example 3, prop_vehicle_jeep rigged model method using
    ...
    local FrontAxle = self:DefineAxle( {
        ...
        Suspension = {
            ...
            MaxTravel = 5,
            ControlArmLength = 22,
            SpringConstant = 31000,
            SpringDamping = 4000,
            --ControlArmLength = 150,
            --SpringConstant = 12530,
            --SpringDamping = 21000,
            SpringRelativeDamping = 0.1,
        },
    } )
